...
class EMMA(nn.Module):  
    def __init__(self, pretrain_model_name_or_path, add_auto_match=True, max_seq_len=128, k=3):
        ...
        self.sen_miu_feature = nn.Linear(self.config.hidden_size, 2*self.config.hidden_size)
        # nn.init.xavier_uniform_(self.sen_miu_feature, gain=0.5)

        # self.sen_logvar_feature = nn.Linear(self.config.hidden_size, 2*self.config.hidden_size)
        # nn.init.xavier_uniform_(self.sen_logvar_feature, gain=0.5)
        
        self.des_e1_extractor = nn.Parameter(torch.ones(max_seq_len, 1))
        # nn.init.normal_(self.des_e1_extractor, mean=0.0, std=0.02)
        self.des_e2_extractor = nn.Parameter(torch.ones(max_seq_len, 1))

        # self.sen_feature = nn.Linear(2*self.config.hidden_size, self.config.hidden_size)
        self.sen_decoder = nn.Linear(2*self.config.hidden_size, self.config.hidden_size)
        self.des_decoder = nn.Linear(self.config.hidden_size, self.config.hidden_size)
        
        # self.relation_encoder = nn.Linear(2*self.config.hidden_size, self.config.hidden_size)

        # self.des_feature = nn.Linear(2*self.config.hidden_size, self.config.hidden_size)
        # nn.init.xavier_uniform_(self.des_feature, gain=0.5)
        ...
        self.lamda3 = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, sen_input_ids, sen_att_masks, des_input_ids=None, des_att_masks=None, \
        ...
        sen_relevant_output = self.sen_miu_feature(sen_output)[:, :, :self.config.hidden_size]
        sen_irrelevant_output = self.sen_miu_feature(sen_output)[:, :, self.config.hidden_size:]
        # print(sen_irrelevant_miu.shape)
        # sen_relevant_logvar = self.sen_logvar_feature(sen_output)[:, :, :self.config.hidden_size]
        # sen_irrelevant_logvar = self.sen_logvar_feature(sen_output)[:, :, self.config.hidden_size:]
        # print(sen_irrelevant_logvar.shape)
        # sen_relevant_feature = self.reparameterize(sen_relevant_miu,sen_relevant_logvar)
        # sen_irrelevant_feature = self.reparameterize(sen_irrelevant_miu,sen_irrelevant_logvar)

        # print(sen_relevant_feature.shape)
        # print(sen_irrelevant_feature.shape)
        sen_vec_gen_feature = self.sen_decoder(torch.cat([sen_relevant_output,sen_irrelevant_output],dim=-1))
        ...
        # sen_vec_feature = torch.mean(sen_relevant_output,dim=1)
        ...
        if des_input_ids != None:
            ...
            # des_miu = self.des_miu_feature(des_output)
            # des_logvar = self.des_logvar_feature(des_output)

            # des_gen_feature = self.reparameterize(des_miu,des_logvar)
            # print(des_gen_feature.shape)
            # des_gen_output = self.des_decoder(des_gen_feature)
        ...
        if des_input_ids != None:
            ...
            des_e1_weight = torch.squeeze(torch.add(torch.matmul(des_e1_weight_matirx,self.des_e1_extractor),self.des_bias1),dim=-1)
            des_e2_weight = torch.squeeze(torch.add(torch.matmul(des_e2_weight_matirx,self.des_e2_extractor),self.des_bias2),dim=-1)
            #聚合步骤
            des_e1_softmax1 = torch.softmax(des_e1_weight,dim=1)
            des_e2_softmax2 = torch.softmax(des_e2_weight,dim=1)


            # print("des_e1_weight:",torch.matmul(des_e1_weight_matirx,self.des_e1_extractor).shape)
            # print("des_e2_weight:",des_e2_weight.shape)

            des_e1_vec = torch.sum(torch.unsqueeze(des_e1_softmax1,dim=-1)*des_output, dim=1)
            des_e2_vec = torch.sum(torch.unsqueeze(des_e2_softmax2,dim=-1)*des_output, dim=1)
            ...
            des_2_sen = self.sen_decoder(torch.cat([des_output,sen_irrelevant_output], dim=-1))
            sen_2_des = self.des_decoder(sen_relevant_output)
            
            sen_vec_feature = torch.mean(sen_relevant_output,dim=1)
            des_vec_feature = torch.mean(des_output,dim=1)
            
            # relation_feature = self.relation_encoder(torch.cat([des_vec_feature,sen_vec_feature], dim=-1))
            ...
            # feature_loss = loss_fct(feature_cos_sim/0.02, con_labels)
            ...
            # loss1 = ori_loss + 0.25*entity_loss + 0.5*feature_loss
            loss1 = ori_loss + 0.5*entity_loss
            # loss1 = ori_loss + 0.25*entity_loss + 0.5*feature_loss
            # loss1 = ori_loss
            # sen_output_cos = self.cos(sen_output,sen_vec_gen_feature)
            # sen_rel_cos = self.cos(des_output,sen_2_des)
            # sen_irrel_cos = self.cos(sen_output,des_2_sen)

            # loss2 = loss_fct(sen_output_cos / 0.02, con_labels)
            # loss3 = loss_fct(sen_rel_cos / 0.02, con_labels) + loss_fct(sen_irrel_cos / 0.02, con_labels)
            loss2 = mse_loss(sen_output,sen_vec_gen_feature)
            loss3 = mse_loss(sen_output,des_2_sen) + mse_loss(des_output,sen_2_des)
            loss = loss1 + 0.25*loss2 + 0.25*loss3
    ...
    def get_des_vec(self, des_output):
        if self.add_auto_match:
            ...
            des_e1 = torch.matmul(des_output, self.des_e1)
            des_e1_weight = torch.squeeze(torch.add(torch.matmul(des_e1, self.des_e1_extractor), self.des_bias1), dim=-1)
            # des_e1 = torch.mean(torch.matmul(des_output, self.des_e1), dim=1)
            # bert_layer1 = torch.squeeze(des_e1, dim=-1) #[bs, ml-1]
            bert_layer_softmax1 = torch.softmax(des_e1_weight, dim=-1) # [bs, ml-1]
            ...
            des_e2 = torch.matmul(des_output, self.des_e2)
            des_e2_weight = torch.squeeze(torch.add(torch.matmul(des_e2, self.des_e2_extractor), self.des_bias2), dim=-1)
            # des_e2 = torch.mean(torch.matmul(des_output, self.des_e2), dim=1)
            bert_layer2 = torch.squeeze(des_e2_weight, dim=-1) #[bs, ml-1]
