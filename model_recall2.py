...
class EMMA(nn.Module):  
    ...
    def forward(self, sen_input_ids, sen_att_masks, des_input_ids=None, des_att_masks=None, \
        ...
        # sen_e1_vec, sen_e2_vec = self.get_sen_entity_vec(sen_output, marked_e1, marked_e2)
        ...
        if des_input_ids != None:
            ...
            # sen_entity_vec = torch.cat([sen_e1_vec,sen_e2_vec],dim=1)
            # des_entity_vec = torch.cat([des_e1_vec,des_e2_vec],dim=1)

            # sen_token_sim_matrix = self.cos(sen_output.unsqueeze(1),des_output.unsqueeze(0)) #[bs,ml,ml]
            # # print(sen_token_sim_matrix.shape)
            # sen_token_score, _ = torch.min(sen_token_sim_matrix,dim=1)
            # sen_token_weight = torch.softmax(sen_token_score, dim=-1)

            des_e1_vec, des_e2_vec = self.get_des_entity_vec(des_output)
            ...
            # 双塔loss
            # sentence_cos_sim = sen_token_weight*sen_token_score
            # entity_cos_sim = self.cos(sen_entity_vec.unsqueeze(1), des_entity_vec.unsqueeze(0)) # [bs, 1, 768]   [1, bs, 768]rf
            # print("cos_sim:",cos_sim.shape)  #10 x 10
            ori_cos_sim = self.cos(sen_vec.unsqueeze(1),des_vec.unsqueeze(0))
            # plt.figure(figsize=(10, 8))
            # sns.heatmap(cos_sim.cpu().detach().numpy(),cmap='viridis', cbar=True)
            # plt.title("vec Heatmap")
            # plt.xlabel("sen ID")
            # plt.ylabel("des ID")
            # plt.savefig("hot_matrix.png", dpi=300, bbox_inches='tight')  # 可以根据需要改变文件名和格式

            con_labels = torch.arange(batch_size).long().to(device)
            loss_fct = nn.CrossEntropyLoss()
            mse_loss = nn.MSELoss().to(device)            
            # loss = loss_fct(cos_sim / 0.02, con_labels)
            # entity_loss = loss_fct(sentence_cos_sim / 0.02, con_labels)
    ...
    def get_des_entity_vec(self, des_output):
        if self.add_auto_match:
            # [bs, ml, hs] x [hs, 1]
