...
class EMMA(nn.Module):  
    def __init__(self, pretrain_model_name_or_path, add_auto_match=True, max_seq_len=128, k=3):
        ...
        self.sen_feature = nn.Linear(self.config.hidden_size,1)
        self.des_feature = nn.Linear(self.config.hidden_size,1)
        ...
        if add_auto_match:
            ...
            self.des_en_bias = nn.Parameter(torch.zeros(max_seq_len, 1))

            self.sen_bias = nn.Parameter(torch.zeros(max_seq_len,1))
            # self.des_bias = nn.Parameter(torch.zeros(max_seq_len, 1))
    ...
    def forward(self, sen_input_ids, sen_att_masks, des_input_ids=None, des_att_masks=None, \
        ...
        # sen_feature = torch.mean(sen_output,dim=1)
        # print(torch.matmul(torch.tanh(sen_feature), self.sen_feature).shape)
        sen_feature_weight = torch.softmax(self.sen_feature(sen_output),dim=-1)

        sen_e1_entity_vec, sen_e2_entity_vec = self.get_sen_entity_vec(sen_output, marked_e1, marked_e2)
        
        sen_entity_between_vec, sen_entity_between_mask = self.get_sen_entity_between_vec(sen_output, marked_e1, marked_e2)

        # sen_entity_between_out = self.sen_en_feature(sen_entity_between_vec)
        # sen_entity_between_feature = F.normalize(sen_entity_between_out,p=2,dim=1) 

       
        sen_eb_vec = torch.mean(sen_entity_between_vec,dim=1)

        sen_entity_vec = F.normalize(torch.cat([sen_e1_entity_vec,sen_e2_entity_vec,sen_eb_vec],dim=1),p=2,dim=1) #[bs,2*hs]
        
        sen_vec_feature = torch.sum(sen_feature_weight*sen_output,dim=1)
        ...
        if des_input_ids != None:
            ...
            des_en_layer = torch.squeeze(torch.add(torch.einsum('bmi,ij,cqj->bcmq',des_output, self.des_en, sen_eb_vec.unsqueeze(1)),self.des_en_bias),dim=-1)
            ...
            des_feature_out = torch.softmax(self.des_feature(des_output),dim=-1)
            des_vec_feature = torch.mean(des_output,dim=1)
            
            # des_feature_out = torch.add(torch.matmul(des_output, self.des_feature), self.des_bias)

            js_loss = self.js_divergence(sen_feature_weight.squeeze(-1), des_feature_out.squeeze(-1))
            ...
            second_largest_indices = feature_topk_indices[:, 1]  # 取每行的第二个最大值的索引
            ...
            for i, ind in enumerate(second_largest_indices):
                ...
                des_neg_list.append(per_des_neg_vec)

            des_neg_vec = torch.stack(des_neg_list,dim=0)
            ...
            loss = ori_loss + js_loss

            # detach: classify模型的梯度不会回传到EMMA，也就是两个模型互不影响
            input_ids, att_masks, token_type_ids, target_idx_arr, vec_idx_arr, sen_vec_, des_vec_, sen_entity_between_mask_ = \
                self.build_classifaction_input(sen_input_ids.detach(), des_input_ids.detach(), sen_output.detach(), des_output.detach(), sen_entity_between_mask.detach(), training=True)
            ...
            classify_loss = self.classify(input_ids.detach(), att_masks.detach(), token_type_ids.detach(), vec_idx_arr.detach(), sen_vec_.detach(), des_vec_.detach(), target_idx_arr.detach(),sen_entity_between_mask_)
            ...
            # sen_vec_feature = torch.mean(sen_output,dim=1)
            # sen_vec_feature = torch.mean(sen_output,dim=1)
            # sen_vec = torch.cat([sen_vec_feature,sen_e1_entity_vec, sen_e2_entity_vec],dim=-1)
            ...
            des_en_layer = torch.squeeze(torch.add(torch.einsum('bmi,ij,cqj->bcmq',self.des_vectors, self.des_en, sen_eb_vec.unsqueeze(1)),self.des_en_bias),dim=-1)
            des_bs_en_weight, _ = torch.max(des_en_layer, dim=1) #[bs_des,des_ml,des_ml]
            des_en_layer_softmax = torch.softmax(des_bs_en_weight,dim=1) #[bs,ml]
            ...
            input_ids, att_masks, token_type_ids, vec_idx_arr, sen_vec, des_vec = \
                self.build_classifaction_input(sen_input_ids, self.des_input_ids_for_predict, sen_vec, self.des_vectors, sen_entity_between_mask,training=False, top_k_indices=top_k_indices)
    ...
    def get_sen_vec(self, sen_output, marked_e1, marked_e2):
        if self.add_auto_match:
            ...
            sen_entity_vec = F.normalize(torch.cat([e1_h,e2_h],dim=1),p=2,dim=1)
    ...
    def get_sen_entity_between_vec(self, sen_output, marked_e1, marked_e2):
        ...
        for i in range(len(e2_idx)):
            if e1_idx[i] < e2_idx[i]:
                ...
                marked_e2[i][e1_idx[i]:e2_idx[i]+1] = 1 
                marked_e2[i][e2_idx[i]] = 0
                
            else:
                marked_e1[i][e2_idx[i]:e1_idx[i]+1] = 1
                marked_e2[i][e2_idx[i]:e1_idx[i]+1] = 1 
                marked_e2[i][e2_idx[i]] = 0
                

        extended_e_mask = marked_e1.unsqueeze(-1)
        # print(extended_e_mask.size()) # [32, 128, 1]
        extended_e_vec = extended_e_mask.float() * sen_output
        # extended_e_mask = torch.mean(extended_e_mask,dim=-2)
        # print(extended_e_mask.size()) # [32, 768]
        # extended_e_mask = torch.stack([sequence_output[i,j,:] for i,j in enumerate(e_mask)])
        # print(extended_e_mask)
        return extended_e_vec.float(), marked_e2      

    def  js_divergence(self,P, Q):
        eps = 1e-14

        P = torch.softmax(P,dim=-1) + eps
        Q = torch.softmax(Q,dim=-1) + eps
        M = (P+Q)/2
    ...
    def build_classifaction_input(self, sen_input_ids, des_input_ids, sen_vec, des_vec, sen_entity_between_mask, training=True, top_k_indices=None):
        '''
        sen_input_ids: [bs, max_seq_length]
        ...
        # print(neg_largestSim_indices)
        device = sen_input_ids.device
        k = self.k
        batch_size = len(sen_input_ids)
        # 选取负样本的策略：随机k个 or vec最接近的k个
        # print(sen_entity_between_mask[0]) #[bs,ml,hs]
        # print(sen_input_ids[0])
        # print((sen_entity_between_mask*sen_input_ids)[0])
        total_sen_tokens = []
        for index, input_ids in enumerate(sen_input_ids):
            entity_between_sen_mask = sen_entity_between_mask[index]
            input_ids_trimmed = self.trim_post_zero(input_ids)
            # # if training:
            # Un_Bet_Ent_input_ids_trimmed = self.mask_entity_bet_vec(input_ids,entity_between_sen_mask)
            # train_input_ids_trimmed = torch.cat([input_ids_trimmed,Un_Bet_Ent_input_ids_trimmed[1:]],dim=0)
            # sen_tokens = self.tokenizer.convert_ids_to_tokens(train_input_ids_trimmed)
            # else:
            sen_tokens = self.tokenizer.convert_ids_to_tokens(input_ids_trimmed)
            # print(Un_Bet_Ent_input_ids_trimmed[1:])
            # entity_between_sen_vec = entity_between_sen_mask
        ...
        for idx in range(batch_size):
            ...
            while(current_idx < k):
                if training:
                    if current_idx == target_idx:
                        ...
                        # per_sen_entity_mask.append(sen_entity_mask[idx])
                        vec_idx_arr_per_sample.append(idx)
                    else:
                        # per_sen_entity_mask.append(sen_entity_mask[idx])
                        # 随机选取负样本
                        neg_idx = random.randint(0, batch_size - 1)
                        # neg_idx = neg_largestSim_indices[idx]
                        # print(idx)
                        # 跳过正样本本身
                        if neg_idx == idx:
                            continue
            ...
            # sen_entity_mask.append(per_sen_entity_mask)
            if training:
        ...
        if training:
            return input_ids, att_masks, token_type_ids, target_idx_arr, vec_idx_arr, sen_vec, des_vec, sen_entity_mask
        else:
    ...
    def mask_entity_bet_vec(self,vec,masked_matrix):
        device = vec.device
        new_vec = vec.clone()
        mask_vec  = vec.clone()
        nonzero_indices = torch.nonzero(new_vec)
        num_nonzero = nonzero_indices.size(0)
        mask_nonzero_ind = torch.nonzero(masked_matrix).squeeze(-1)
        mask_sen_vec = mask_vec[mask_nonzero_ind[0]:mask_nonzero_ind[-1]+1]
        new_vec[mask_nonzero_ind] =  self.tokenizer.mask_token_id
        input_ids_trimmed = torch.cat([new_vec[:num_nonzero],mask_sen_vec,torch.tensor([self.tokenizer.sep_token_id]).to(device)],dim=0)
...
class Classify_model(nn.Module): 
    ...
    def forward(self, input_ids, att_masks, token_type_ids, vec_idx_arr, sen_vec_arr, des_vec_arr, target_idx_arr=None, sen_entity_between_mask = None):
        '''
        input_ids: [bs, k, ml]
        att_masks: [bs, k, ml]
        token_type_ids: [bs, k, ml]
        ...
        #sen_between_vec [bs,hs]
        x = self.mlp1(bert_output) # [bs, hs]

        # sen_bet_vec = torch.mean(sen_between_vec,dim=1)
        x = torch.relu(x)
        # class_vec = torch.cat([x,sen_bet_vec],dim=-1)
        
        # class_vec = self.mlp2(class_vec)
        # class_vec = torch.relu(class_vec)
