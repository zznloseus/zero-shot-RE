...
class Classify_model(nn.Module): 
    def __init__(self, pretrain_model_name_or_path, max_seq_len=128, k=3):
        ...
        self.entity_mlp1 = nn.Linear(k*self.config.hidden_size,128)
        self.entity_mlp2 = nn.Linear(128,self.config.hidden_size)
    ...
    def forward(self, input_ids, att_masks, token_type_ids, vec_idx_arr, sen_vec_arr, des_vec_arr, span_mask, target_idx_arr=None):
        ...
        cls_feature = self.cls_mlp1(bert_cls_f)
        cls_feature = torch.relu(cls_feature)
        cls_feature = self.cls_mlp2(cls_feature)
        cls_feature = torch.relu(cls_feature)
        ...
        logits1 = self.cls_mlp3(cls_feature*entity_feature)
