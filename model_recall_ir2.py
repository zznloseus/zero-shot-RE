...
class EMMA(nn.Module):  
    def __init__(self, pretrain_model_name_or_path, add_auto_match=True, max_seq_len=128, k=3):
        ...
        self.des_miu_feature = nn.Linear(self.config.hidden_size, self.config.hidden_size)
        self.des_logvar_feature = nn.Linear(self.config.hidden_size, self.config.hidden_size)
    ...
    def forward(self, sen_input_ids, sen_att_masks, des_input_ids=None, des_att_masks=None, \
        ...
        if des_input_ids != None:
            ...
            sen_vec_feature = torch.mean(sen_relevant_output,dim=1)
            sen_vec = sen_vec = torch.cat([sen_vec_feature, sen_entity_vec], dim=1) #[bs,3*bs]
            des_batch_size = self.des_output_infer.size(0)

            # for bs in range(0, batch_size, des_batch_size):
            des_e1_weight_matirx = torch.einsum('bmi,ij,bqj->bmq',self.des_output_infer, self.des_e1, sen_output) #[bs,ml,ml]
            des_e2_weight_matirx = torch.einsum('bmi,ij,bqj->bmq',self.des_output_infer, self.des_e2, sen_output) #[bs,ml,ml]
            ...
            des_e1_vec = torch.sum(torch.unsqueeze(des_e1_weight,dim=-1)*self.des_output_infer, dim=1)
            des_e2_vec = torch.sum(torch.unsqueeze(des_e2_weight,dim=-1)*self.des_output_infer, dim=1)

            des_entity_vec = F.normalize(torch.cat([des_e1_vec,des_e2_vec],dim=1),p=2,dim=1) #[bs,2*hs]
            des_vec_feature = torch.mean(self.des_output_infer,dim=1)
            ...
            self.des_vectors = torch.cat([des_vec_feature ,des_entity_vec], dim=1)
            sen_vec = torch.cat([sen_vec_feature, sen_entity_vec], dim=1) #[bs,3*bs]
    ...
    def gen_des_vectors(self, des_features):
        ...
        self.des_output_infer = des_output
