...
class EMMA(nn.Module):  
    def __init__(self, pretrain_model_name_or_path, add_auto_match=True, max_seq_len=128, k=3):
        ...
        # self.des_miu_feature = nn.Linear(self.config.hidden_size, 2*self.config.hidden_size)
        # nn.init.xavier_uniform_(self.sen_logvar_feature, gain=0.5)
        
        # self.des_e1_extractor = nn.Linear(self.config.hidden_size, 1)
        # self.des_e2_extractor = nn.Linear(self.config.hidden_size, 1)
        ...
        if add_auto_match:
            self.des_weights1 = nn.Parameter(torch.ones(self.config.hidden_size, 1))
            # self.des_bias1 = nn.Parameter(torch.zeros(max_seq_len - 1, 1))
            self.des_bias1 = nn.Parameter(torch.zeros(max_seq_len, max_seq_len))
            self.des_weights2 = nn.Parameter(torch.ones(self.config.hidden_size, 1))
            # self.des_bias2 = nn.Parameter(torch.zeros(max_seq_len - 1, 1))
            self.des_bias2 = nn.Parameter(torch.zeros(max_seq_len, max_seq_len))
    ...
    def forward(self, sen_input_ids, sen_att_masks, des_input_ids=None, des_att_masks=None, \
        ...
        if des_input_ids != None:
            ...
            # sen_gen_output = self.sen_decoder(torch.cat([sen_relevant_output, sen_irrelevant_output],dim=-1))
            ...
            # des_relevant_output = self.des_miu_feature(des_output)[:, :, :self.config.hidden_size]
            # des_irrelevant_output = self.des_miu_feature(des_output)[:, :, self.config.hidden_size:]
            # des_vec_gen_feature = self.des_decoder(self.reparameterize(des_relevant_output,des_irrelevant_output))
            ...
            # feature_cos_sim = torch.mean(self.cos(sen_relevant_output.unsqueeze(1),des_output.unsqueeze(0)),dim=-1)
            # print(cos_sim) 10 x 10
            # print(feature_cos_sim.shape)
            ...
            # loss1 = ori_loss + 0.1*entity_loss
            # loss1 = ori_loss + 0.5*feature_loss
            loss1 = ori_loss
            ...
            # print(self.KL_divergence(des_relevant_output,des_irrelevant_output))
